    ...
    <div class="form-group mt-4" @if (isset($parcel_type->id) && $parcel_type->dimensions == 1) @else style="display:none" @endif>
        ...
        <div class="card dimentions">
            <label for="height">
                ...
                <span>Height</span>
                <input type="number" min="1" class="text big_parcel_charges"
                    value="{{ isset($detail->height) ? $detail->height : '' }}" data-id="0" name="height[]"
                    id="height_0" value="" placeholder="">
            </label>
            <label for="width">
                <img src="{{ asset('assets/svg/width.svg') }}" alt="">
                <span>Width</span>
                <input type="number" min="1" class="text big_parcel_charges" data-id="0" name="width[]"
                    id="width_0" value="{{ isset($detail->width) ? $detail->width : '' }}" placeholder="">
            </label>
            <label for="length">
                <img src="{{ asset('assets/svg/length.svg') }}" alt="">
                <span>Length</span>
                <input type="number" min="1" class="text big_parcel_charges" data-id="0" name="length[]"
                    id="length_0" value="{{ isset($detail->length) ? $detail->length : '' }}" placeholder="">
            </label>
        </div>
    </div>
